...
if run:
    ...
    def render_portfolio(name, weights, ret, vol, sharpe):
        st.subheader(name)
    ...
    with left:
        render_portfolio("⭐ Maximum Sharpe portfolio", max_sharpe.x, ms_ret, ms_vol, ms_sharpe)
    with right:
        render_portfolio("🛡️ Minimum volatility portfolio", min_vol.x, mv_ret, mv_vol, mv_sharpe)
